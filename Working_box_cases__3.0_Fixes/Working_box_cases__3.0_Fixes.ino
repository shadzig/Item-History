...
#define STRIP_PIN 6                                         // Strip pin number
#define NUM_STRIP_LEDS 8                                    // Number of NEOPIXELS

Adafruit_NeoPixel strip = Adafruit_NeoPixel(NUM_STRIP_LEDS, STRIP_PIN, NEO_GRB + NEO_KHZ800); // STRIP SETTINGS & VALUES

#include <pcmConfig.h>                                      // SD storage library and Audio recording / speaker
#include <pcmRF.h>
#include <TMRpcm.h>
#include <SD.h>
#include <SPI.h>
...
#define SD_ChipSelectPin 10                                 //Chip select is pin number 53 for MEGA and 10 for UNO
...
//void PRESSURE_SENSOR() {                                         // Analog pressure not working as Microphone recording uses uses analog INPUT limit
//  LEDdata = analogRead(pressurePin);
//  //Serial.println(LEDdata);
//  if (LEDdata > pressure)
//  {
//    digitalWrite(LEDpin, HIGH);                                 // PCB LED response to see changes
//    force = HIGH;
//  }
//  else
//  {
...
void PRESSURE_SENSOR_DIGITAL() {                                   // Digital pressure sensor code due to limit of 1 analog input from arduino UNO, so using digital resistance measure instead
  ...
  if  (fsrReading < pressure) {
    digitalWrite(LEDpin, HIGH);                                   // PCB LED response to see changes
    force = HIGH;                                                 // Tracking pressure value and changing it to a digital response for case changes
  }
  else
  {
    digitalWrite(LEDpin, LOW);
...
void maintainDisplay(int event)                                 // Debuging display feed. Every 1 second
{
...
void refreshDisplay(int event)
{
  ...
  //  Serial.print("Pressure value: ");                          // Analog pressure read
  //  Serial.println(LEDdata);
  Serial.print("RCtime reading = ");
  Serial.println(fsrReading);                                    // Raw digital reading
...
void loop() {
  ...
  unsigned long now = millis();                                         // Time value for NEOPixel flash animation
  int t = now % 1000;
  ...
  switch (state) {                                                      // Call mode 0 on start up
    ...
    case ACCEPT:
      ...
      for (int i = 0; i < NUM_STRIP_LEDS; i++) {
        strip.setPixelColor(i, map(t, 0, 999, COLOUR1_R, COLOUR2_R),            // Flashing Neo Pixel colour WHITE to BLUE mapping
      ...
      if (event == REMOVE) {                                                    //  Iteem removed transition event
        state = IDLE_STATE;
      ...
      if (event == TIME_OUT) {
        ...
        Timer_Running = true;
        digitalWrite(3, HIGH);                                                  // LED PCB response (when serial feed is not visiblw
        Serial.println("startrecording IN");                                    // Recording start
    ...
    case RECORDING_MODE:
      ...
      if (event == REMOVE) {
        state = IDLE_STATE;
        Serial.println("IDLE_STATE");
        Serial.println("stoprecording IN");                                // Recording stop
      ...
      if (event == TIME_OUT) {
        ...
        Target_Time_Millis = millis() + long_timer;                        // Setting timer for next state (Case)
        Timer_Running = true;
        Serial.println("stoprecording IN");                                // Recording start
        audio.stopRecording("1.wav");
        Serial.println("stoprecording OUT");
        digitalWrite(3, LOW);
      }
