...
const int RECORDING_MODE = 3;
const int WAITING_PICKUP = 4;
const int PLAYBACK = 5;
const int PLAYBACK_PAUSE = 6;
...
const int instruction_timer = 6000;
const int long_timer = 32000;
...
#define COLOUR1_B 115

#define COLOUR2_R 20
#define COLOUR2_G 112                       // BRIGHT BLUE RGB
#define COLOUR2_B 92

#define COLOUR3_R 100
#define COLOUR3_G 0                       // RED RGB
#define COLOUR3_B 5

#define COLOUR4_R 0
#define COLOUR4_G 180                     // MINT GREEN RGB
#define COLOUR4_B 60

#define COLOUR5_R 30
#define COLOUR5_G 10                     // PURPLE RGB
#define COLOUR5_B 100
  ...
  switch (state) {                                                      // Call mode 0 on start up
    ...
    case ACCEPT:
      ...
      if (event == TIME_OUT) {
        ...
        Target_Time_Millis = millis() + instruction_timer;                        // Setting timer for next state (Case)
        Timer_Running = true;
        Serial.write(mystr1, 2); //Write the serial data: "play instructions"
      }
      break;
    ...
    case INSTRUCTIONS:
      ...
      for (int i = 0; i < NUM_STRIP_LEDS; i++) {
        ...
        // pixels.Color takes RGB values, from 0,0,0 up to 255,255,255
        strip.setPixelColor(i, strip.Color(90, 100, 5)); // Moderately bright RED color.
      ...
      if (event == REMOVE) {
        state = IDLE_STATE;
        //Serial.println("IDLE_STATE");
        Serial.write(mystr2, 2); //Write the serial data: "Stop instructions"
      }
      ...
      if (event == TIME_OUT) {
        ...
        Serial.write(mystr2, 2); //Write the serial data: "Stop instructions"
        delay(1000);
        Serial.write(mystr3, 2); //Write the serial data: "start recording"
      }
      break;
    ...
    case RECORDING_MODE:
      ...
      for (int i = 0; i < NUM_STRIP_LEDS; i++) {
        strip.setPixelColor(i, map(t, 0, 999, COLOUR1_R, COLOUR3_R),            // Flashing Neo Pixel colour WHITE to BLUE
      ...
      if (event == REMOVE) {
        state = IDLE_STATE;
        //Serial.println("IDLE_STATE");
        Serial.write(mystr4, 2); //Write the serial data: "Stop recording"
      }
    ...
    case WAITING_PICKUP:
      ...
      for (int i = 0; i < NUM_STRIP_LEDS; i++) {
        ...
        // pixels.Color takes RGB values, from 0,0,0 up to 255,255,255
        strip.setPixelColor(i, strip.Color(0, 180, 60)); // Moderately bright GREEN color.
      ...
      if (event == REMOVE) {
        ...
        Target_Time_Millis = millis() + long_timer;                        // Setting timer for next state (Case)
        Timer_Running = true;
        Serial.write(mystr5, 2); //Write the serial data: "Start playback"
      }
      break;
    ...
    case PLAYBACK:
      ...
      for (int i = 0; i < NUM_STRIP_LEDS; i++) {
        strip.setPixelColor(i, map(t, 0, 999, COLOUR1_R, COLOUR4_R),          // Flashing Neo Pixel colour WHITE to GREEN
                            map(t, 0, 999, COLOUR1_G, COLOUR4_G),
                            map(t, 0, 999, COLOUR1_B, COLOUR4_B));
      ...
      if (event == ADD) {
        ...
        Serial.write(mystr6, 2); //Write the serial data: "Stop playback"
      }
